    ...
    <div class="stack">
      ...
      <section class="panel">
        ...
        <div class="runtime-grid">
          ...
          <div class="status-row with-icon"><div class="status-icon">${icon("search")}</div><div><div class="row-title">Semantic file matching</div><div class="row-subtitle">${data.runtime.semanticResourceRetrievalEnabled === false ? "Falls back to exact filename and lexical matching." : `Uses ${escapeHtml(data.runtime.semanticResourceEmbeddingModel || "local embeddings")} with minimum score ${escapeHtml(data.runtime.semanticResourceMinScore ?? "0.72")}.`}</div></div>${pill(data.runtime.semanticResourceRetrievalEnabled === false ? "disabled" : "enabled", data.runtime.semanticResourceRetrievalEnabled === false ? "neutral" : "ok")}</div>
