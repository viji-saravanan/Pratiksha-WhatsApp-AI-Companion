...
- The local store path can be forced through `VIJI_WACLI_STORE`, keeping WhatsApp state under the configured Pratiksha data root.
...
```bash
wacli version --json
wacli doctor --json --store "/data/pratiksha/wacli/store" --timeout 5s
```

Observed on 2026-05-02:

- `wacli` version: `0.6.0`
- Store path: `/data/pratiksha/wacli/store`
