...
- Complete: 19 phases, Phase 0 through Phase 16 plus Phase 18 and Phase 19.
- In progress: 1 numbered phase, Phase 17.
- Planned: 7 numbered phases, Phase 20 through Phase 26.
- Remaining: Phase 17 production-contact trial follow-up plus Phase 20 through Phase 26.
...
- Phase 20 through Phase 26 are future implementation phases created from the repo-wide review in [14_REPO_WIDE_REVIEW_AND_FUTURE_PHASES.md](14_REPO_WIDE_REVIEW_AND_FUTURE_PHASES.md).
...
- [ ] Project data stays under `/Volumes/Arya 1TB/VijiAI`.
...
| Phase 20 | Persistent WhatsApp adapter spike | Planned | Not started; target is socket/event-stream behavior behind the adapter boundary |
| Phase 21 | Automatic received-media persistence | Planned | Not started; target is unattended download/promotion of allowlisted media |
...
- [ ] Update adapter spike docs with direct `wacli`, direct `whatsmeow`, and bridge options.
- [ ] Design event-stream interface without bypassing policy/outbox.
- [ ] Define auth/session storage under `VIJI_DATA_ROOT`.
- [ ] Define reconnect, receipt, media, and message event handling.
- [ ] Keep `wacli` as rollback until implementation is proven.

### Phase 21 Checklist

Goal: automatically save allowlisted received media.

- [ ] Add Compose-managed media drain path.
- [ ] Keep media downloads quota-controlled.
- [ ] Clean partial files on failure.
- [ ] Add dashboard and CLI media queue status.
- [ ] Test image, document, audio, duplicate, non-allowlisted, group, and storage-full cases.
...
corepack pnpm exec playwright screenshot --wait-for-timeout 3000 --full-page --browser chromium --viewport-size 1440,1100 http://127.0.0.1:8788 /Users/aryasubramani/Downloads/viji-dashboard-runtime.png
node scripts/generate-project-skills.mjs
corepack pnpm skills:validate
<temporary SSD venv>/bin/python /Users/aryasubramani/.codex/skills/.system/skill-creator/scripts/quick_validate.py /Users/aryasubramani/.codex/skills/viji-helper-dev
```
