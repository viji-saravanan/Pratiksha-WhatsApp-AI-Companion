...
| Phase 20 | Planned | Persistent WhatsApp adapter spike for socket/event-stream behavior |
| Phase 21 | Planned | Automatic received-media persistence in unattended Docker runtime |
...
- Active source workspace at `/Volumes/Arya 1TB/VijiAI/workspace/viji-helper`.
- Runtime data root at `/Volumes/Arya 1TB/VijiAI`.
- Sentinel file at `/Volumes/Arya 1TB/VijiAI/.viji-helper-root`.
...
- Media storage under `/Volumes/Arya 1TB/VijiAI/wacli/media`.
...
- Model paths under `/Volumes/Arya 1TB/VijiAI/models`.
...
- This proves the current `wacli` runtime works, but also confirms the remaining latency limitation: fresh intake is bounded by the scheduled sync interval plus one-shot sync duration. Phase 20 is still required for persistent-socket behavior.
