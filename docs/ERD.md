...
| `kb_embedding_model_updated_at` | timestamptz |  |
...
| `target_kb_document_chunk_id` | uuid fk nullable | `kb_document_chunks.kb_document_chunk_id` |
| `target_res_resource_id` | uuid fk nullable | `res_resources.res_resource_id` |
| `model_kb_embedding_model_id` | uuid fk | `kb_embedding_models.kb_embedding_model_id` |
| `kb_embedding_vector` | vector | pgvector column |
| `kb_embedding_dimensions` | integer | Stored separately so mixed local models can coexist |
| `kb_embedding_content_hash` | text | Hash of the embedded resource text or chunk content |
| `kb_embedding_created_at` | timestamptz |  |
| `kb_embedding_updated_at` | timestamptz |  |

Constraints:

- Exactly one target is set: either `target_kb_document_chunk_id` or `target_res_resource_id`.
- Unique index on `(target_kb_document_chunk_id, model_kb_embedding_model_id)` for chunk embeddings.
- Unique index on `(target_res_resource_id, model_kb_embedding_model_id)` for resource metadata embeddings.
- HNSW cosine index is migration-controlled for 1024-dimension local embeddings; other dimensions continue to work through the exact vector operator until a model-specific index is added.
...
| `kb_retrieval_chunk_created_at` | timestamptz |  |

Constraints:

- Exactly one target is set: either `target_kb_document_chunk_id` or `target_res_resource_id`.
- Retrieval run ranks are unique per run.
