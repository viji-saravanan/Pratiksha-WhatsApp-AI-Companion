...
test("dashboard Compose profile owns API and Postgres lifecycle", async () => {
  ...
  assert.equal(
    ...
    "postgres://pratiksha:change-me-postgres-password@postgres:5432/pratiksha"
  );
...
test("documented environment defaults use Compose service DNS for container networking", async () => {
  ...
  const local = await readOptionalFile(".env");

  assert.match(example, /VIJI_DASHBOARD_DOCKER_API_BASE_URL=http:\/\/api:8787/);
  assert.match(
    example,
    /DATABASE_DOCKER_URL=postgres:\/\/pratiksha:change-me-postgres-password@postgres:5432\/pratiksha/
  );
  ...
  if (local.length > 0) {
    ...
    assert.match(local, /DATABASE_DOCKER_URL=["']?postgres:\/\/[^"'\s]+@postgres:5432\/[^"'\s]+["']?/);
...
test("live Compose profile owns the WhatsApp automation worker lifecycle", async () => {
  ...
  assert.equal(
    liveWorker.environment.DATABASE_URL,
    "postgres://pratiksha:change-me-postgres-password@postgres:5432/pratiksha"
  );
  ...
  assert.equal(liveWorker.environment.VIJI_LIVE_POLL_INTERVAL_MS, "3000");
